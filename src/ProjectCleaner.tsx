...
import { FolderOpenOutlined, ScanOutlined, DeleteOutlined } from '@ant-design/icons';
...
const ProjectCleaner: React.FC = () => {
  ...
  const handleScanProjects = async () => {
    ...
    try {
      ...
      if (result.success && result.projects) {
        ...
        message.success(`Scan completed, found ${result.projects.length} projects`);
      } else {
  ...
  const handleDeleteNodeModules = async () => {
    ...
    const selectedProjects = projects.filter((p) => selectedRowKeys.includes(p.path));
    const totalSize = calculateTotalSize(selectedProjects);

    setCleaning(true);
    try {
      const result = await window.electronAPI.deleteNodeModules(selectedRowKeys);

      if (result.success) {
        // Check specific project cleanup results
        const successfulDeletions = result.results?.filter((r) => r.success).length || 0;
        const failedDeletions = result.results?.filter((r) => !r.success).length || 0;

        // Update project status
        const updatedProjects = projects.map((project) => {
          if (selectedRowKeys.includes(project.path)) {
            const operationResult = result.results?.find((r) => r.path === project.path);
        ...
        if (failedDeletions === 0) {
          message.success(`Cleanup successful! Freed ${formatFileSize(totalSize)} of space`);
        } else {
          message.warning(
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a: Project, b: Project) => a.nodeModulesSize - b.nodeModulesSize,
    },
    {
      title: 'Last Modified',
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Card>
        ...
        <div>
          <Text type="secondary">
            Scan all Node.js projects in the specified directory, analyze the size of their
            node_modules directories and provide cleanup functionality
          </Text>
        </div>
        ...
        {projects.length > 0 && (
          <div style={{ marginBottom: '16px' }}>
            <Space>
              <Checkbox
                checked={selectedRowKeys.length === projects.filter((p) => p.hasNodeModules).length}
                indeterminate={
                  selectedRowKeys.length > 0 &&
                  selectedRowKeys.length < projects.filter((p) => p.hasNodeModules).length
                }
              ...
              <Text>
                Selected {selectedProjects.length} projects, estimated to free{' '}
        ...
        <Spin spinning={scanning}>
          <Table
            ...
            scroll={{ x: 800 }}
            locale={{
              emptyText: 'No project data, please select a directory and scan first',
            }}
          />
        </Spin>
      </Card>
