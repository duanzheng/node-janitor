...
async function detectPackageManagerCaches(): Promise<Cache[]> {
  ...
  try {
    ...
    // Fallback: try common pnpm store locations under the user's home directory
    try {
      const homeDir = app.getPath('home');
      const candidates = [
        path.join(homeDir, 'Library', 'pnpm', 'store'),
      ...
      if (fallbackPath) {
        const size = await getDirectorySize(fallbackPath);
        caches.push({
          name: 'pnpm',
          path: fallbackPath,
          size,
