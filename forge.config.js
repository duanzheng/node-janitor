...
const hasSigningIdentity = Boolean(process.env.CSC_NAME || process.env.CSC_IDENTITY);
...
module.exports = {
    ...
    postMake: async (_forgeConfig, makeResults) => {
      ...
      for (const artifact of artifacts) {
        try {
          ...
          await execa('xcrun', ['stapler', 'staple', path.resolve(artifact)], { stdio: 'inherit' });
        } catch {
  ...
  packagerConfig: {
    ...
    ...(isMac && hasSigningIdentity
      ...
      : {}),
    // 不再生成通用二进制，避免 CI 上合并 x64/arm64 失败
